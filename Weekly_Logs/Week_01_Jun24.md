...
2. **The Hessian Term**: This is a geometric correction factor (the inverse square root of the normal Hessian). It adjusts the probability density based on how sharply the space curves away from the manifold at that specific point $u$. 
3. **Intrinsic Measure**: The $d\mathcal{M}(u)/du$ ensures the maths respects the natural, non-Euclidean curvature of the manifold. 
...
**Line:**
"Let $\Omega\subset\mathbb{R}^{m}$ be an open set and let $\Omega^{\prime}\subset\Omega$ be a closed ball. Let $c_{1}:=\text{Vol}(\Omega^{\prime})$."
...
$$
F|_{\Omega^{\prime}} \in C^{3}(\Omega^{\prime}) \text{ and } F \ge 0 \text{ on } \Omega. \text{ There is a unique minimiser } x^* \in \text{int}(\Omega^{\prime}) \text{ of } F \text{ on } \Omega.
$$
...
$$
m_{1} := \inf_{x\in\Omega\setminus\Omega^{\prime}}\{F(x)-F(x^*)\} > 0, \quad m_{2} := \inf_{x\in\Omega^{\prime}}\lambda_{\min}(\nabla^{2}F(x)) > 0
$$

*(Note: The text briefly contains a typo calling the first term n_1, but later refers to it correctly as m_1).*

* **The Maths ($m_1$):** This looks at all points *outside* our bubble. It states that the value of $F(x)$ everywhere outside the bubble is strictly greater than the minimum $F(x^*)$ by at least a gap of $m_1$. This guarantees there are no "competing" peaks elsewhere in the domain that could mess up the integral.
...
**Line:**
"Let $c_{4}:=\sup_{x\in\Omega^{\prime}}|g(x)| , c_{5}:=\sup_{x\in\Omega^{\prime}}\|\nabla g(x)\| , c_{6}:=\int_{\Omega}|g(x)|dx.$"
...
2. **The Volume Term:** The term with $(2\pi\theta)^{m/2}$ over the square root of the Hessian determinant represents the volume or "width" of the peak. When you integrate a Gaussian, the area depends on the variance. Here, $\theta$ acts like the variance. The sharper the bowl, the smaller the volume under the peak. 
...
* **The Maths:** They take the target function $f_{\theta}$ and expand it into its three known parts: $f_{0}$, $f_{1}$, and $\hat{f}$. Because these parts are added inside an exponent, they can be separated into multiplied exponential terms.
...
#### 3. The Proof: Step 2 - The Plus-One / Minus-One Trick
...
**Line:** "For the first term, we can directly apply Corollary B.1 with $F(r)=f_{0}(u,r)$, $g(r)=\exp(-f_{1}(u,r))h(u,r)$, and $\Omega^{\prime}$ being the ball $\{r \mid \|r\|\le\hat{\epsilon}\}$."
...
* **The Maths:** They are trying to find the absolute maximum possible size of the second (bin) integral. They pull the amplitude $h$ and the error factor outside the integral by replacing them with their maximum possible values (the supremum, or $\sup$).
...
* **The Maths:** The error term $\overline{f}$ vanishes completely (goes to $o(1)$) as the parameter $\theta$ goes to zero. It does this evenly (uniformly) everywhere inside the tubular neighbourhood around the manifold.

**Line:** "3. The density concentrates in $T_{\mathcal{M}}(\epsilon)$, i.e.,"

$$
\lim_{\theta\rightarrow 0}\int_{T_{\mathcal{M}}(\epsilon)}\exp\left(-\frac{f_{\theta}(x)}{\theta}\right)dx = 1
$$

* **The Maths:** They assume it is already established that 100% of the probability mass will eventually end up somewhere inside the tubular neighbourhood ($T_{\mathcal{M}}(\epsilon)$). 

---

#### 2. The Core Claim

**Line:** "For any $\eta>0$, define the set $C_{\eta}=\{x \mid f_{0}(x)>\eta\}$. Then,"

$$
\int_{C_{\eta}\cup T_{\mathcal{M}}(\epsilon)^{c}}\exp\left(-\frac{f_{\theta}(x)}{\theta}\right)dx \rightarrow 0 \text{ as } \theta\rightarrow 0
$$

* **The Maths:** This is the hypothesis they intend to prove. They define a region $C_{\eta}$. This region is everywhere that $f_0(x)$ is greater than some tiny positive number $\eta$—meaning it is everywhere *except* exactly on the manifold. They claim that the probability of finding a data point in this region (or completely outside the tubular neighbourhood) shrinks to exactly zero.

**Line:** "If in addition, $\exp(-f_{\theta}(x)/\theta)$ converges weakly to a distribution as $\theta\rightarrow 0$, the support of the limiting distribution is contained in $\mathcal{M}$."

* **The Concept:** If the function stabilises into a final distribution, that distribution will be entirely supported on the manifold $\mathcal{M}$.

---

#### 3. The Proof: Bounding the Probability

**Line:** "Proof. Since we have that $\int_{T_{\mathcal{M}}(\epsilon)}\exp(-f_{\theta}(x)/\theta)dx\rightarrow 1$ for the first result, it suffices to show that $\int_{T_{\mathcal{M}}(\epsilon)\cap C_{\eta}}\exp(-f_{\theta}(x)/\theta)dx\rightarrow 0$."

* **The Maths:** Because Assumption 3 already established the mass is trapped inside the tubular neighbourhood, they only need to evaluate the space *inside* the tube but *off* the manifold ($T_{\mathcal{M}}(\epsilon)\cap C_{\eta}$). If they can prove the integral over that specific area goes to zero, the proof is complete.

**Line:** "According to the assumptions, we have that for any $\delta>0$, $\exists\theta_{0}$, such that $\forall\theta<\theta_{0}, |\overline{f}(x,\theta)|<\delta$. Therefore, we have"

$$
\int_{T_{\mathcal{M}}(\epsilon)\cap C_{\eta}}\exp\left(-\frac{f_{\theta}(x)}{\theta}\right)dx \le \int_{T_{\mathcal{M}}(\epsilon)\cap C_{\eta}}\exp\left(\frac{-\eta+\delta}{\theta}\right)dx \le \text{Vol}(T_{\mathcal{M}}(\epsilon))\exp\left(\frac{-\eta+\delta}{\theta}\right)
$$

* **The Maths:** Since the error term $\overline{f}$ vanishes, they can bound its maximum absolute value by a tiny number, $\delta$. Because they are evaluating the region where $f_0 > \eta$, they can bound the exponent by its worst-case scenario: $(-\eta + \delta)/\theta$. They pull this exponential term out of the integral, leaving only the finite volume of the tubular neighbourhood.

**Line:** "We choose $\delta=\eta/2$ then the right-hand side goes to zero as $\theta\rightarrow 0$."
...
**Line:** "Let the limiting measure be $P$, and $P_{\theta}$ be the probability measure corresponding to the density $\exp(-f_{\theta}(x)/\theta)$. Since $C_{\eta}$ is an open set, we have that"

$$
P(C_{\eta}) \le \liminf_{\theta\rightarrow 0}P_{\theta}(C_{\eta}) = 0
$$

* **The Maths:** They transition from evaluating densities to evaluating the final probability measure $P$. Using the Portmanteau theorem for weak convergence, the final probability of an open set is less than or equal to the limit infimum of the approaching probabilities. Since they just proved the approaching probabilities go to 0, the final probability must also be exactly 0. 

**Line:** "Denote $C:=\mathcal{M}^{c}.$ We have that $C=\cup_{m=1}^{\infty}C_{1/m}\cup\overline{T_{\mathcal{M}}(\epsilon)}^{c}$. Then we have"

$$
P(C) \le \sum_{m=1}^{\infty}P(C_{1/m}) + P(\overline{T_{\mathcal{M}}(\epsilon)}^{c}) = 0
$$

"which concludes the proof."

* **The Concept:** They define $C$ as the entire complement of the manifold ($\mathcal{M}^{c}$). They represent this space as a countable union of all the bounded regions off the manifold ($C_{1/m}$) plus the space completely outside the tubular neighbourhood. Since they proved the probability measure of any of those individual regions is exactly 0, their sum also equals 0. 
...
**Line:** "Theorem B.1. Assume Assumption B.1. Define $\pi_{\theta}(x)\propto \exp\left(-\frac{f_{\theta}(x)}{\theta}\right)$ ,"

* **The Maths:** The theorem begins by inheriting all the structural rules established in Assumption B.1 (such as a compact $C^4$ manifold and a strictly positive Hessian for $f_0$). It then defines the target probability density $\pi_{\theta}(x)$, which is proportional to the exponential of the expanded function $f_{\theta}(x)$ scaled by the parameter $\theta$.

**Line:** "Assume that $1-\int_{x\in T_{\mathcal{M}}(\epsilon)}\pi_{\theta}(x)dx\rightarrow 0$ as $\theta\rightarrow 0$."

* **The Maths:** This establishes a prerequisite condition: as $\theta$ approaches zero, the total probability mass strictly outside the tubular neighbourhood $T_{\mathcal{M}}(\epsilon)$ drops to zero. This ensures that 100% of the relevant density is concentrated near the manifold, validating the use of local coordinates for the remainder of the analysis.

---

#### 2. The Core Claim and Formula

**Line:** "Then we have that as $\theta\rightarrow 0$, $\pi_{\theta}$ converges weakly to the following distribution:"

* **The Concept:** This states the primary result. The high-dimensional, noisy density $\pi_{\theta}$ stabilises (converges weakly) into a defined distribution strictly supported on the manifold surface.

**Line:** $$
\pi(u) = \frac{\exp(-f_{1}(u,0))\left|\frac{\partial^{2}f_{0}(u,0)}{\partial r^{2}}\right|^{-1/2}d\mathcal{M}(u)/du}{\int_{\mathcal{M}}\exp(-f_{1}(u,0))\left|\frac{\partial^{2}f_{0}(u,0)}{\partial r^{2}}\right|^{-1/2}d\mathcal{M}(u)/du}
$$

* **The Maths:** This is the formula for the limiting distribution. It consists of a numerator (the unnormalised density at a specific point $u$ on the manifold) and a denominator (the normalisation constant ensuring the total probability integrates to 1). 
* **The Breakdown of the Numerator:**
    ...
    * **Geometric Correction**: The term involving the inverse square root of the normal Hessian is the geometric correction factor derived directly from Lemma B.1. It adjusts the probability density based on how sharply the ambient space curves into the manifold at that specific location.
...
**Line:** "where $d\mathcal{M}$ is the intrinsic measure on the manifold $\mathcal{M}$, i.e., $d\mathcal{M}(u)=|g(u)|^{1/2}du$, and $du$ is the Lebesgue measure on the local parameterisation domain $U$."
...
* **The Concept:** The authors address a minor technical discrepancy. Hwang originally assumed his starting function was already fully normalised. The authors are working with an unnormalised density that requires a denominator. However, they assert that the underlying calculus and limit techniques of Hwang's proof are mathematically valid regardless of whether the function is pre-normalised or normalised at the end.
...
* **The Maths:** The theorem grounds itself in the foundational rules established earlier in the paper: the data lives on a smooth, compact manifold (Assumption 2.1) and possesses a smooth, strictly positive probability density (Assumption 2.2). The equation is specifically evaluated for points $x$ that are situated inside the tubular neighbourhood ($T_{\mathcal{M}}(\epsilon)$), meaning they are relatively close to the manifold surface.
...
**Line:** $$
\log p_{\sigma}(x) = -\frac{1}{\sigma^{2}}d_{\mathcal{M}}(x) + \log p_{data}(\Phi^{-1}(P_{\mathcal{M}}(x))) - \frac{d-n}{2}\log(2\pi\sigma^{2}) + H(x) + o(1)
$$
...
* **Term 1: The Geometric Pull** Here, $d_{\mathcal{M}}(x)$ is the squared distance from your given point $x$ to the closest point on the manifold. Because it is divided by $\sigma^{2}$, as $\sigma \rightarrow 0$, this term explodes toward $-\infty$ for any point not perfectly resting on the manifold. This acts as a massive mathematical gravity well, forcing the probability mass to concentrate strictly on the geometry of the manifold. Because it scales by $\sigma^{-2}$, it is the "loudest" and most dominant signal for the model to learn.

* **Term 2: The Data Distribution** This represents the actual density of the original data, evaluated at the specific point on the manifold closest to $x$ (the projection $P_{\mathcal{M}}(x)$ mapped back to local coordinates). Crucially, there is no $\sigma$ in this term. It is $\Theta(1)$, meaning its strength remains constant regardless of the noise level.

* **Term 3: The Perpendicular Volume** This acts as the normalisation factor for the Gaussian noise. The quantity $d-n$ represents the number of ambient dimensions that are strictly perpendicular to the manifold. This term accounts for the volume of the noise dispersed out into those off-manifold directions.
...
* **Term 5: The Vanishing Error ($o(1)$)**
  **Line:** "The small $o(1)$ term is uniform for $x\in T_{\mathcal{M}}(\epsilon)$."
  ...
  This is a mathematically tiny remainder that uniformly shrinks to zero across the entire tubular neighbourhood as the noise parameter $\sigma$ approaches zero.
